...
def read_fields_be(path: str, n: int) -> list:
    """Read n big-endian 32-byte field elements, return as hex strings."""
    with open(path, 'rb') as f:
        data = f.read()
    return [f'"0x{int.from_bytes(data[i*32:(i+1)*32], "big"):064x}"' for i in range(n)]
