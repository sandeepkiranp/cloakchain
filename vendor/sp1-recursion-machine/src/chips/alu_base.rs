        ...
        values[..populate_len].par_chunks_mut(NUM_BASE_ALU_ACCESS_COLS).zip_eq(instrs).for_each(
            |(row, instr)| {
                ...
                *access = BaseAluAccessCols {
                ...
                // SP1 6.2.3 bug: DivF rows with mult=0 (disabled/padding) can have
                // in2=0 and in1≠0 in memory, making the constraint
                // `is_div * (in2 * out - in1) = 0` unsatisfiable (0*out ≠ in1).
                // Fix: leave all flags at false (is_real=0) for disabled DivF rows so
                // the constraint and memory receives are fully gated off, turning the
                // row into a true no-op. The executor writes an all-zero event for
                // these rows (see sp1-recursion-executor patch).
                if *opcode == BaseAluOpcode::DivF && mult.is_zero() {
                    return; // is_add=is_sub=is_mul=is_div=0 already set above
                }
