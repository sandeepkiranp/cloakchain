...
impl RecursionVks {
    ...
    pub fn open(
        ...
        let index = if self.vk_verification {
            ...
            let index = self.map.get(&digest).copied().ok_or_else(|| {
                if std::env::var("RECURSION_DIAG").map(|v| v == "1").unwrap_or(false) {
                    let digest_u32: Vec<u32> =
                        digest.iter().map(|f| f.as_canonical_u32()).collect();
                    eprintln!(
                        "[VK-DIAG] vk not allowed: digest={digest_u32:?}  num_registered_vks={}",
                        self.map.len(),
                    );
                }
                TaskError::Fatal(anyhow::anyhow!("vk not allowed"))
            })?;
            index
        } else {
