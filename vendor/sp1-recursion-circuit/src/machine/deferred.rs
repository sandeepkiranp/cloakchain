    ...
    A: MachineAir<SP1Field> + for<'a> Air<RecursiveVerifierConstraintFolder<'a>>,
{
    ...
    pub fn verify(
        ...
        for (deferred_idx, (vk, shard_proof)) in vks_and_proofs.into_iter().enumerate() {
            ...
            // RECURSION_DIAG: same idea as compress.rs, marker range 800000000+deferred_idx.
            // This is the check for EXTERNALLY-supplied deferred proofs (e.g. the previously
            // compressed inner coin-proof / vfy-g16 proof passed in via write_proof), so a
            // mismatch here would point to a stale/foreign vk_root baked into that earlier proof.
            if std::env::var("RECURSION_DIAG").map(|v| v == "1").unwrap_or(false) {
                builder.print_debug(800_000_000 + deferred_idx);
                for elem in current_public_values.vk_root.iter() {
                    builder.print_f(*elem);
                }
                for expected in vk_root.iter() {
