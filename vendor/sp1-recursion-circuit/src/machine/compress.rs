    ...
    A: MachineAir<InnerVal> + for<'a> Air<RecursiveVerifierConstraintFolder<'a>>,
{
    ...
    pub fn verify(
        ...
        for (i, (_, shard_proof)) in vks_and_proofs.into_iter().enumerate() {
            ...
            // RECURSION_DIAG: dump (proof index, expected vk_root, actual vk_root) to stdout
            // via the DSL `print` opcode so the mismatched shard proof can be identified from
            // proof-batch position rather than raw ASM addresses. Marker range 700000000+i
            // distinguishes this loop from the deferred-verifier's own check (see deferred.rs).
            if std::env::var("RECURSION_DIAG").map(|v| v == "1").unwrap_or(false) {
                builder.print_debug(700_000_000 + i);
                for expected in vk_root.iter() {
                    builder.print_f(*expected);
                }
                for actual in current_public_values.vk_root.iter() {
                    builder.print_f(*actual);
                }
            }
