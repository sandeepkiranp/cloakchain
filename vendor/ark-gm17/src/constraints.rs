...
impl<E: Pairing, P: PairingVar<E>> SNARKGadget<E::ScalarField, E::BaseField, GM17<E>>
    for GM17VerifierGadget<E, P>
{
    ...
    fn verify_with_processed_vk(
        ...
        let one = P::GTVar::one();
        Ok(test1.is_eq(&one)? & test2.is_eq(&one)?)
    }
    ...
    P: PairingVar<E>,
{
    ...
    fn to_bytes_le(&self) -> Result<Vec<UInt8<E::BaseField>>, SynthesisError> {
        ...
        for q in &self.query {
            bytes.extend_from_slice(&q.to_bytes_le()?);
        }
        Ok(bytes)
    }
}
