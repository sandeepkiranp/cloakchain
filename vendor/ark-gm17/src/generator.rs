    ...
    R: Rng,
{
    ...
    let g_num_scalars =
        ...
        // G gamma2 Z t
        + m_raw + 1;
    let g_table = BatchMulPreprocessing::new(g, g_num_scalars);
    ...
    let a_query = g_table.batch_mul(&cfg_iter!(a).map(|a| *a * &gamma).collect::<Vec<_>>());
    end_timer!(a_time);
    ...
    let g_gamma2_z_t = g_table.batch_mul(
    ...
    let c_query_2 = g_table.batch_mul(
        &cfg_into_iter!(0..sap_num_variables + 1)
    ...
    let h_gamma_table = BatchMulPreprocessing::new(h_gamma, non_zero_a);
    end_timer!(h_gamma_time);

    // Compute the B-query
    let b_time = start_timer!(|| "Calculate B");
    let b_query = h_gamma_table.batch_mul(&a);
    drop(h_gamma_table);
    end_timer!(b_time);
    ...
    let vk = VerifyingKey::<E> {
        ...
        query: verifier_query.to_vec(),
    };

    // `BatchMulPreprocessing::batch_mul` (0.5's replacement for the old
    // `FixedBase::msm`) already returns affine points directly, unlike the
    // old API which needed a separate `normalize_batch` projective->affine
    // conversion pass here — that pass is dropped, not translated.
    let c_query_1 = c_query_1.to_vec();
